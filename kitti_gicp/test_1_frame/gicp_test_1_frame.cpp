...
int main(int argc, char** argv)
{
	...
	//读取pcd文件
	if (pcl::io::loadPCDFile<pcl::PointXYZ>("/media/chenxin/我不是硬盘/kitti_dataset/sequences/05/pcd/0.pcd", *source) == -1)
	{
	...
	if (pcl::io::loadPCDFile<pcl::PointXYZ>("/media/chenxin/我不是硬盘/kitti_dataset/sequences/05/pcd/0.pcd", *target) == -1)
	{
    ...
    */
	...
	gicp.setInputCloud(source_downsampled);
	...
	icp.setInputCloud(source_downsampled); //设置输入点云
